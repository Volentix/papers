...
בעת יישום פרוטוקול הלופרינג FIFO (הראשון-להיכנס הראשון-לצאת) בbuffer מעגלי, צמתי-רשת יכולים לעצב את ספרי ההזמנות שלהם בהתאם להזמנת המשתמש. שיטה זו הולכת אחרי מודל OTC, שם מגבלת ההזמנות מוגדרת בהתבסס על מחיר בלבד.[5]

בהתייחסות אל התמדת הAPI של EOS.IO, ספר ההזמנות יכול יהיה לנצל את מיכל המולטי-אינדקס (טבלה המאפשרת גישה ישירה לרשומות על-פי מפתח) המשותף בין צמתים דרך אותו חשבון EOS.IO.
...
שימוש באופציית ריבוי-כניסות בתוך ''ארנק חם'' בעל מספר תהליכים/שרתים עצמאיים אשר בודקים פעמיים כל משיכה שמתבצעת, יחדו גם ההטבה הנלווית של יצירת רשימה בטוחה של חשבונות.

פריסתו של חוזה מותאם המאפשר משיכות אך ורק לחשבונות מאומתים על-ידי KYC/AML.

פריסתו של חוזה מותאם המאשר אך ורק פיקדונות של נכסים ידועים מחשבונות מאומתים על-ידי KYC/AML.

פריסתו של חוזה מותאם אשר דורש תקופת המתנה של 24 שעות עבור כל משיכה.

השמשת חוזים עם ארנקי חומרה לכלל הכניסות, כולל משיכות אוטומטיות.
...
#### 3.8.2 ביקורת תהליכים חריגים

האלגוריתמים המתקדמים לזיהוי חריגה שמסופקים על-ידי ''Numenta’s Nupic library'', ידרשו פחות ערכות נתונים לאימונים נרחבים מאשר מערכות קונבנציונליות של AI (בינה מלאכותית).

#### 3.8.3 אקראיות

אקראיות אמיתית של פריסת מרחב הכתובות, וניתוב או ניצול אורכים משתנים של פסקי-זמן כפי שמתרחש בפרוטוקול הRAFT, יכולה להפוך מערכות למורכבות יותר, עד כדי העצמת החוסן והאבטחה של מערכות מפני תקיפות מסוג side-channed או front-running.

#### 3.8.4 ביקורת יומן כניסות

Parsec אינו צריך להסתמך על רישומי כניסה בכדי לקבוע הסכמה. מצד שני, RAFT ידרוש זאת. בכל מקרה, צמתי רשת מאמתים ידרשו מידע מינימלי על מנת לקבוע הסכמה בנושא צמצום זמן תהליך ההעברה.

#### 3.8.5 העברה כאלגוריתם הPOS

מנגנון זה מונע העברה חוזרת מבוססת-Fork ( פיצול של מטבע לתתי מטבע וחלוקתם לבעלי המטבעות המקוריים הנוצרים בעת שינויי חוק בפרוטוקול הבלוקצ’יין) אשר לא כוללים את הבלוק המיוחס. מסמן לרשת כאשר משתמש מסוים וה-Pos שלו נמצאים על Fork ספציפי.

### 3.9 איומים בטחוניים ופתרונם
...
התקפה של 51% יכולה להתבצע על-ידי כל אחד בעל 50% מסך כוח המחשוב של רשת. מרבית מעמדת הבעלות מאפשרת היפוך של כל העברה ומתירה שליטה מוחלטת במבחר של ההעברות המופיעות בבלוקים. EOS.IO, פרוטוקול הלופרינג והRAFT מונעים את בעיה זו. אם מפיק בלוק מנצל כמות לא סבירה של זמן-ריצה או אינו מספיק רווחי, התהליך יופיע ברשימה השחורה.[5]

#### 3.9.2 גניבת הזמנות

Front runner גונב הזמנה אחת או יותר מספר ההזמנות הנמצא בהמתנה בעסקת הסדר. EOS.IO ופרוטוקול הלופרינג מציעים שניהם פתרונות, באמצעותן המפתחות מוגנים, מפני שאינם לוקחים חלק בהעברות על הבלוקצ'יין, ולפיכך נותרים לא ידועים. לצומת המייסד בספר ההזמנות יש מידע רגיש באופן בלעדי, כמו כן כל צומת משתמש בשיטת פיתרון אחרת עבור תיקון ספרי ההזמנות, מה שמציג בפנינו עוד רמת מורכבות עבור קידום ביטחון.

#### 3.9.3 זיוף זהויות

משתמשים עם כוונות זדוניות יכולים ליצור זהויות מזויפות על מנת לשלוח מספר רב של הזמנות קטנות הנועדו לתקוף ''Loopring nodes''. עם זאת, מרבית ההזמנות האלו יידחו על אי-מתן רווח מספק כשיותאמו.

#### 3.9.4 יתרה לא מספיקה

משתמשים בעלי כוונות זדוניות עלולים להיכנס ולהפיץ הזמנות, כאשר ערכן שונה מאפס אך הכתובת שממנה נשלחת ההזמנה היא בעלת יתרה של אפס. צמתי רשת מפקחים על היתרה בזמן אמת, מעדכנים את מצבי ההזמנות בהתאם, ולאחר מכן מוחקים אותם.

#### 3.9.5 התקפות מתוזמנות

התקפות מתוזמנות הן סוג של התקפות הצפנה, בהן צד חיצוני משקיף יכול לעיין בתכני הנתונים המוצפנים ולחשבם (על אף ההצפנה) על-ידי רישום וניתוח הזמן שנלקח להוציא לפועל את אותם אלגוריתמים מוצפנים. אלגוריתם הRAFT מונע התקפות מתוזמנות באמצעות שימוש בפסקי-זמן אקראיים.
...
EOS.IO הינה תשתית קוד פתוח, הניתנת להרחבה בעבור יישומים מבוזרים. מטרתה היא תהליך בחירות הוגן ושקוף של מפיק הבלוק (BP) הנבנה על הסכמה נציגית דמוקרטית של אלגוריתם הPOS. במיוחד כאשר מערכת שכזו רק מתחילה להתרבות, ישנו סיכוי לתקלות. משום כך, מידה מסוימת של מרכוז שמור הינו בלתי נמנע והכרחי. הפילוסופיה המנחה שלנו היא ''ביזור'', והמאמצים המתמשכים שלנו ממוקדים לצמצום התלות בסמכות מרכזית.
...
פרוטוקול הRAFT והPARSEC מספקים שניהם מערכת חזקה עבור שחזור במקרה של כשל צמתי (node failure). אמצעי ביטחון מסופקים גם כן בעבור המסחר בין בלוקצ'יינים מקומיים. אם שרשרת מסרבת לתהליך האימות, המערכת נאלצת לעבור אל הבלוק הבא כברירת מחדל, או להציב נעילה לזמן קצר.

### 3.13 התפתחות העיצוב המבני

הכרזות יומיות על פיתוח של קודים חדשים, המשפיעים על השימוש בנכס הדיגיטלי, אשר חושפות את יתרונה העצום של פילוסופיית הקוד הפתוח. אנחנו בVolentix יודעים שאנו המרוויחים העיקריים מהמשאבים הכספיים העצומים שהוקדשו על ידי משקיעים מתחילים רבים, על מנת לפתח יישומי נכסים דיגיטליים בעשור האחרון. כעת יש בידינו הזדמנות לעשות את הצעד הבא באמצעות פלטפורמת הVDex, ''חלפן כספים'' מבוזר בעבור הדור הבא של העסקאות הדיגיטליות.

## 4. מחשבות לסיכום

כולנו פה בVolentix מקדישים את העבודה והתובנה לפיתוח תוכנה המבוססת על העצמה ועצמאות. אם ברצונך להצטרף אלינו, בכל קיבולת שהיא, עשה/י זאת וכך גם תשכיל/י יותר על הנושאים הנכללים בתוך ה''רשימה הלבנה'' ופרסומים נוספים של Volentix תוך כדי שיתופם עם קהילתנו.
...
"הרשימה הלבנה" נוצרה, ומיוצגת עבור מטרות מידע בלבד. המידע המוצג לנ''ל אינו מתיימר להיות מקיף. מידע זה כפוף לשינויים מלאים או חלקיים העלולים לקרות בכל עת ללא התראה מוקדמת. מעבדות Volentix שומרות לעצמן את הזכות לשנות, להחליף, להסיר, או למחוק חלק ו/או כל מידע על פי שיקול דעתן הבלעדי. מעבדות Volentix אינן לוקחות אחריות או חלק ייצוגי, מובע או מרומז, בדבר הדיוק או השלמות של המידע, ומתנערת במפורש מכל אחריות שהיא, בהקשר המידע הכלול או האינו כלול. מעבדות Volentix מבקשות מכל קורא לעיין ולקרוא את מלוא המידע בקפידה, ולהתחייב לבדיקה וניתוח מידע עצמאי. כמו כן לחפש ולהשיג עצות מקצועיות למטרות של הערכת המידע. למיטב ידיעת מעבדות Volentix, אין אף סוכנות רגולטורים, ממשלה, או ישות אכיפה אחרת מצד שלישי שבדקה, העריכה, או אישרה חלק ו/או כל המידע. מידע זה אינו הצעה או שידול בכל צורה שהיא, ואינו יוצר בסיס לחוזה או התחייבות מכל סוג שהוא. כל הצהרה הנחשבת כצופה לעתיד היא עניין של דעה בלבד, ואין להסתמך על הצהרה שכזו או על כל חלק מהמידע הרשום בה בכל צורה שהיא.
...
4. (2017) blockchainhub.net, blockchain-oracles.

5. F. Zhou, Wang, Loopring: פרוטוקול חלפן כספים מבוזר, (2018).
...
8. D. Eosio.boot, Larimer צ'אט בטלגרם, (2018).

9. J. O. Diego Ongaro, בחיפוש אחר אלגוריתם קונצנזוס הניתן להבנה, (2018).
...
12. צוות BlockCollider, הרשימה הלבנה של Block collider, (2018).

## אזכורים נוספים

Aelf, מסגרת מחשוב בלוקצ'ייני רב-שרשורית מקבילה, (2018).

ARK, פלטפורמת הסתגלות לצרכן, (2018).
...
M. R. Garrick Hileman, מחקר השוואתי גלובלי על מטבעות-קריפטו, (2017).

Komodo, טכנולוגיית בלוקצ'יין מתקדמת, המתרכזת בחופש, (2018).
...
G. Wood, אתריום: לדג'ר העברות מאובטח, כללי ומבוזר. פרויקט "הרשימה הצהובה" של אתריום, (2014).
