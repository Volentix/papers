...
هذا الموديل مسؤول عن التواصل مع كل من العُقد المنشورة لكل سلسلة يجرى تقييمها. ويتمثل دوره في توفير واجهة موحدة ومتماسكة للطبقة النهائية من نوع RESTful بحيث يمكن استخلاص طبيعة أي سلسلة كتل مع معرفة كيفية الاستعلام عن أي عُقدة كاملة يتم تشغيلها بواسطة النظام، وهذا لأجل جمع المعلومات حول السلسلة وخصائص الشبكة التابعة لتلك السلسلة. في كثير من الحالات، لا تعمل العملة الرمزية القابلة للتداول على سلسلة خاصة بها ولكن يتم تطبيقها سريعًا باستخدام نظام من العُقود التي يتم تشغيلها باستخدام سلسلة تسمح بإستخدام العُقود الذكية مثل Ethereum. في هذه الحالة، يتواصل هذا الموديل مع العقدة الكاملة لهذه السلسلة والموديلات الأخرى، على سبيل المثال، موديل التحليل الثابت Ethereum المذكور لاحقاً في هذا المستند.

يمكن تطبيق مجموعة واسعة من التكنولوجيا على هذه الطبقة، ولكن نظراً لمحدودية العديد من التقنيات المختلفة اللازمة لفهم النظام بالكامل واستخدام التقنيات التي يتم نشرها على نطاق واسع، اختيار تقنية مماثلة مثل الطبقة النهاية من نوع RESTful التي تتفاعل مع هذا الموديل سيكون عاقلاً. نموذج **node.js-based** سيكون إختياراً حكيماً، رغم أن تقنيات التنفيذ الأخرى ممكنة بالتأكيد. كذلك، توفر Ruby on Rails طريقة سريعة للغاية لإنشاء واجهات برمجة تطبيقات REST أيضًا.

للحصول على المعلومات الأكثر فائدة حول سلسلة معينة، من الضروري عادة تشغيل عُقدة كاملة تراقب كل كتلة قادمة. عند القيام بذلك، يمكن أن تتوفر معلومات في الوقت الفعلي عن أي من الإحصائيات المهمة، بما في ذلك ارتفاع السلسلة، إستنتاج متوسط وقت الكتل ومتوسط وقت تشغيلها، أحجام الكتل، عدد المعاملات وأي عدد من الخصائص الأخرى المعروضة لأي عُقدة كاملة. إن واجهة الاستعلام عن هذه العُقدة الكاملة هي قرار اتخذه مصممو العميل المعني بالأمر، وبالتالي فهي مسؤولية موديل إحصائيات السلسلة لتتوافق مع هذه الواجهة وتحويلها إلى نموذج يمكن أن تستهلكه الموديلات العليا التي لا تفضل واجهة محددة.

عدد كبير من العملات الرمزية التي يتم تداولها اليوم، هي عملات ERC20 التي تم تطبيقها على رأس سلسلة كتل Ethereum. علاوة على ذلك، فإن العديد من العملات الرمزية المثيرة هي جزء من نظام أكبر من العُقود الذكية التي تستخدم عملات ERC20 كعملة أصلية خاصة بها. تقديم تحليل متعمق لهذه العُقود وأي نظام عقد ذكي يعمل على رأس االألة الإفتراضية الخاصة بـ Ethereum مفيد للغاية لتحقيق أهداف Vespucci الكبيرة والمنشودة. وبالتالي، يوجد هذا الموديل لاستهلاك أنظمة العُقود الذكية المرتبطة ببعض عملات ERC20 وذلك لأجل تصنيفها على مستوى الشفرة، على مستوى السلامة العامة، وجود الأخطاء، والعوامل المهمة الأخرى التي يمكن التحقق منها بتحليل ثابت لشفرة العَقد في نظام العُقود الذكية. Much of its core logic is written in Python, and bridges are developed to the blockchain statistics module.

* ### **تجربة المستخدم**

لدى Vespucci واجهة مستخدم (UI) تسمح بإدخال الموسط مباشرة وتعرض بشكل متناسق مقاييس التقييم الناتجة. توفر واجهة الإدخال قائمة جميع العملات الإلكترونية المتاحة أو مجموعة مختارة من العملات الأعلى تصنيفا وفقا للقيمة السوقية: يمكن للمستخدم اختيار العملات التي تهمه/تهمها. عوضاً عن ذلك، يُطلب من المستخدم توفير رابط سلسلة عامة واحدة (أو أكثر) خاصة بعملة إلكترونية معينة بالإضافة إلى إدخلات أساسية تتمثل في رابط شفرة مصدر المشروع. يتم اختيار نوع ومستوى التحليل المراد تنفيذه على العملة المحددة. يتطلب التحليل الأكثر تفصيلا المزيد من المدخلات. ‬

توفر واجهة النتائج طريقة عرض مطوية حيث توفر فهرسًا لمختلف الأقسام القابلة للتوسيع والتي تعرض المقاييس المحددة بأتمّ التفاصيل. تعرض طريقة العرض المطوية ملخص تصنيف عملة إلكترونية مشفر بإستخدام مختلف الألوان. يتم حساب التصنيف المشفر باستخدام متوسط مرجّح لجميع مقاييس التقييم. يعرض كل قسم قابل للتوسيع المقياس أو مجموعة المقاييس بطريقة رسومية. يتم تجزئة مقاييس التقييم إلى أقسام مختلفة بناءً على المعلومات الخاصة بها مثل البيانات المتعلقة بالمنظور الاجتماعي أو الفني. يتم تحديد الفهرسة الأمثل لمقاييس التقييم لكى يسمح بتفاعل المستعمل مع واجهة المستخدم البديهية للوصول إلى البيانات التي تهمه ولعرضها.

بعد نجاح Ethereum Wallet ومتصفح Ethereum Mist، سيتم استضافة الواجهة الرئيسية المكشوفة للمستخدم داخل تطبيق Electron. Electron هي تقنية ناضجة مقدمة من قِبل Github لإنشاء تطبيقات الكمبيوتر الحديثة. بالرغم من أن Vespucci غير مرتبط بواجهات برمجة التطبيقات المرتكز على الكمبيوتر، إلا أنه من الممكن النفاذ إلى هذا التطبيق من خلال المتصفح. يمكن تصميم واجهة التطبيق باستخدام مكتبات واجهة المستخدم الناضجة للتطبيقات الحديثة. نظرًا لإستقلال كل المكونات، من السهل سحب وإضافة مكونات جديدة وبالتالي يتم تجربة المنتج مع المستخدمين دون التأثير سلبًا على المنتج ككل.

ميزات المنصة

* إظهار جميع الموسطات التي تم استخدامها لحساب درجة Vespucci.
* قدرة المستخدم على تعديل الموسطات الأساسية لتكوين الدرجة.
* عرض المرشحات للعملات الإلكترونية.
